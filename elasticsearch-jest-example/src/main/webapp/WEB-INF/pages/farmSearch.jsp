...
<head>
    ...
    <link href="/statics/css/pager.css" rel="stylesheet" type="text/css" />
    <script src="/statics/js/jquery.pager.js" type="text/javascript" charset="utf-8"></script>
    <style>
        .bs-docs-footer {
            padding-top: 40px;
            padding-bottom: 40px;
    ...
    <div class="search">
        <div class="container">
            <section>
                <div class="row-fluid">
                    <div>
                        <form id="farmSearch" name="farmSearch" action="/farm/search" class="form-inline" method="post">
                            <div class="form-group">
                                <select class="form-control" name="field">
                                    <option value="all" <c:if test="${field==\"all\"}">selected</c:if>>全部</option>
                                    <option value="title" <c:if test="${field==\"province\"}">selected</c:if>>省份</option>
                                    <option value="content" <c:if test="${field==\"market\"}">selected</c:if>>市场</option>
                                    <option value="source" <c:if test="${field==\"type\"}">selected</c:if>>品类</option>
                                    <option value="author" <c:if test="${field==\"name\"}">selected</c:if>>品种</option>
                                    <option value="author" <c:if test="${field==\"time\"}">selected</c:if>>时间</option>
                                </select>
                            </div>
                            <div class="form-group">
            ...
            <c:if test="${!empty count}">
                <h2><span class="label label-success">搜索关键字<em>${queryString}</em>共检索到<em>${count}</em>条记录，共<em>${totalPages}</em>页，每页显示<em>${pageSize}</em>条。</span></h2>
            </c:if>
        </div>
    </div>
</div>
<div class="content">
    <div class="container">
        ...
        <c:forEach items="${farms}" var="farm">
            <div>
                <h3><span>时间：${farm.time}   省份：${farm.province}   市场：${farm.market}   品类：${farm.type}   品种：${farm.name}   颜色：${farm.color}   最低价格：${farm.minPrice} 平均价格：${farm.avgPrice} 最高价格：${farm.maxPrice}</span></h3>
            </div>
        </c:forEach>
...
<div id="full" style="width:0px; height:0px; position:fixed; right:70px; bottom:150px; z-index:100; text-align:center; background-color:transparent; cursor:pointer;">
    <a href="#" onclick="goTop();return false;"><img src="/statics/images/top.png" width="70px;" height="70px;" border=0 alt="回到顶部"></a>
</div>
<script src="/statics/js/top.js" type="text/javascript"></script>
<!--返回顶部结束-->
    ...
    $("#pageNav").pager(
            {
                ...
                buttonClickCallback:function(pageCurrent) {
                    ...
                    document.getElementById('farmSearch').submit();
                }
            });
});
</c:if>
